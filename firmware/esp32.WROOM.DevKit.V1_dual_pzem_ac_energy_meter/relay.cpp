...
String status_json() {
  // Sized for RELAY_COUNT entries of ~7 fields each.
  ...
  for (uint8_t r = 0; r < RELAY_COUNT; ++r) {
    ...
    o["sm"]            = sm_str(s_r[r].sm);
    o["latest_w"]      = s_r[r].power_valid ? (int)(s_r[r].latest_power_w + 0.5f) : -1;
    o["compressor_w"]  = s_r[r].compressor_watts;
    o["grace_min"]     = s_r[r].grace_min;
    o["sched_version"] = s_r[r].version;
  }
  String out;
