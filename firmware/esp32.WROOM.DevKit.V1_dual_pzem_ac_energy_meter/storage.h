...
struct RowFields {
  uint64_t seq;
  // 1-based PZEM channel this row was measured on (1..PZEM_CHANNELS). One
  // sampling instant emits one row per channel, all sharing the same `seq` —
  // so (seq, channel) identifies a row, and seq-based acking/truncation still
  // advances both channels together.
  uint8_t  channel;
  uint32_t boot_id;
...
// schedule would re-fire a second later, in a loop.
...
// mis-schedule its reboot. All are a no-op on an unchanged value, to limit flash
...
// Per-channel: `ch` is 0-based here (0 == PZEM-1), matching the pzem:: API.
