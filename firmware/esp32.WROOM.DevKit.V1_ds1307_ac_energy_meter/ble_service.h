...
// radio during a connect + TLS sync. Not needed on the dual-core WROOM
// (WIFI_PAUSE_BLE_DURING_SYNC = 0) and kept only for parity with the
// single-radio boards. pause_advertising() only stops advertising; an
...
// Wi-Fi. Used at the BLE→Wi-Fi handoff on single-radio boards. NOT used on the
// WROOM (BLE_CONFIG_WINDOW_SEC = 0 keeps BLE up for the whole run), and kept
// only for parity. Irreversible until reboot; tick()/is_alive() become safe
// no-ops afterward.
void shutdown();
