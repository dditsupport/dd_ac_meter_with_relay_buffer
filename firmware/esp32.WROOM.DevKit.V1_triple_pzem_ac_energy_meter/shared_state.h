...
#include "config.h"   // PZEM_CHANNELS sizes the per-channel array below
...
// Everything measured per PZEM lives here; SharedState holds one of these per
// channel. Splitting it out is what lets the two meters keep independent
// totals, anchors and fault status instead of sharing one set of fields.
struct ChannelState {
  PzemSample latest{};
...
// Global state and mutex (defined in ac_energy_meter.ino).
