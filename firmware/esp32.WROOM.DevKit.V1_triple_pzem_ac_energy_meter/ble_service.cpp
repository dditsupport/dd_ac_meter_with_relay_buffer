...
static String build_device_info_json() {
  // Sized for the base fields PLUS the per-channel array (PZEM_CHANNELS x ~5
  // fields). The single-meter build fit in 384 B; the channels array pushes
  // past that, and an overflowing StaticJsonDocument silently truncates.
  StaticJsonDocument<512 + PZEM_CHANNELS * 160> doc;
  ...
  doc["relay_count"]   = relay::count();
  {
    ...
    if (state_snapshot(snap)) {
      ...
      for (uint8_t i = 0; i < PZEM_CHANNELS; ++i) {
        JsonObject o = chs.createNestedObject();
        o["ch"]        = i + 1;                       // 1-based on the wire
        o["total_kwh"] = snap.ch[i].total_kwh;
        o["today_kwh"] = snap.ch[i].today_kwh;
        o["power_w"]   = snap.ch[i].latest.power;
        o["status"]    = (uint8_t)snap.ch[i].pzem_status;
        total_all += snap.ch[i].total_kwh;
      }
      // Sum across meters, so a client that doesn't understand channels still
      // gets a meaningful single number.
      doc["total_kwh"] = total_all;
    }
  }
...
class PzemResetCallbacks : public NimBLECharacteristicCallbacks {
  void onWrite(NimBLECharacteristic *c, NimBLEConnInfo &info) override {
    ...
    // Optional 1-based "ch": reset just that meter. Omitted (or 0) resets every
    // channel, which is what a plain fresh-install reset wants.
    int ch = doc["ch"] | 0;
    if (ch == 0) {
      pzem::request_reset(pzem::PZEM_RESET_ALL);
      LOG_PRINTLN("[ble] energy-register reset requested by app (all channels)");
    } else if (ch >= 1 && ch <= PZEM_CHANNELS) {
      pzem::request_reset((uint8_t)(ch - 1));
      LOG_PRINTF("[ble] energy-register reset requested by app (ch%d)\n", ch);
    } else {
      LOG_PRINTF("[ble] pzem-reset bad channel: %d\n", ch);
    }
  }
};
