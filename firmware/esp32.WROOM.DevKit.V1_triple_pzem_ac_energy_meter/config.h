...
// ---------- Identity / build ----------
// 3.1.0 is the TRIPLE-PZEM build: one ESP32 reads PZEM_CHANNELS meters and
...
// 1 = serial logging on, for bench testing. 0 = production: the LOG_* macros
// compile to nothing, Serial.begin() is skipped, the serial command console is
// omitted, and log_serial::init() actively silences the ESP-IDF's own logging,
// so no code path can write to or read from UART0 except the PZEM driver.
//
// This ALSO selects the meter count, because UART0 can only serve one master —
// see the PZEM channel section below. Set 1 to debug (2 meters), 0 to ship (3).
...
// UART0 is shared between the serial console and PZEM-3, so the two are
// mutually exclusive and DEBUG_SERIAL decides which one gets it:
//   DEBUG_SERIAL 1 (bench testing) -> UART0 is the console, PZEM-3 is OFF,
//                                     so this build runs as a 2-meter unit.
//   DEBUG_SERIAL 0 (production)    -> UART0 is PZEM-3's Modbus bus, no console,
//                                     full 3-meter unit.
// Making the flag pick the channel count (rather than leaving both enabled)
// removes the footgun: logging and Modbus can never end up sharing the line.
// The firmware reports its live channel_count/relay_count over BLE and in every
// POST, so the app and the cloud follow this automatically — flip the flag,
// reflash, and both ends show 2 or 3 meters with no other change.
#if DEBUG_SERIAL
  #define PZEM_CHANNELS         2
  #warning "DEBUG_SERIAL=1: UART0 is the serial console, so PZEM channel 3 is DISABLED (2-meter build). Set DEBUG_SERIAL 0 for the full 3-meter build."
#else
  #define PZEM_CHANNELS         3
#endif
...
// Channel 3 on UART0 — the ESP32's third and last hardware UART, and the one
// the bootloader flashes over. Fine for a soldered-down module: the programmer
// is only attached on the bench and PZEM-3 only in the field, so they never
// share the line at once.
//
// The runtime consequence is that UART0 is now a Modbus bus, so THIS BUILD HAS
// NO SERIAL CONSOLE AND NO LOGGING (DEBUG_SERIAL below is 0). Anything printed
// to UART0 would land mid-transaction and corrupt PZEM-3's replies, and
// anything READ from it would steal those replies' bytes. Flash the dual-PZEM
// build when you need serial diagnostics.
//
// The ROM bootloader still emits its own banner on TX0 at every reset, before
// our code runs. PZEM-3 sees that as a malformed frame and ignores it.
#define PIN_PZEM3_RX            3         // ESP32 RX0 <- PZEM-3 TX
#define PIN_PZEM3_TX            1         // ESP32 TX0 -> PZEM-3 RX
#define PZEM3_ADDR              0xF8
...
#define PIN_RELAY3              25              // cuts the AC measured by PZEM 3
