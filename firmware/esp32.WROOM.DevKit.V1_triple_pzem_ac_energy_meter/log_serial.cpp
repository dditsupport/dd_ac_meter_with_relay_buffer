...
// Swallow everything routed through esp_log. Used when DEBUG_SERIAL == 0.
...
#if !DEBUG_SERIAL
  // UART0 is PZEM-3's Modbus bus in this build, so NOTHING may write to it.
  // Our own LOG_* macros already compile away, but the ESP-IDF logs on its own
  // (wifi/lwip/NimBLE chatter) and would corrupt Modbus transactions. Silence
  // every route to the port: esp_log at source, its vprintf sink, and the
  // ROM-level ets_printf/putchar path that bypasses esp_log entirely.
