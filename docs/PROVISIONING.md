...
- **USB CDC On Boot: Enabled** *(mandatory — the Super Mini has no USB-UART
  bridge, so the serial console runs over native USB. With this disabled you
  will see no boot logs.)*
...
| `RTClib` (Adafruit) | 2.1.x | DS1307 RTC |
...
1. Open `firmware/esp32.supermini_ds1307_ac_energy_meter/esp32.supermini_ds1307_ac_energy_meter.ino` in Arduino IDE.
...
Watch the serial monitor at 115200 baud (the USB-CDC port the board
enumerates as). Within a few seconds you should see the boot banner, the
device ID, and live PZEM readings (zero if no load) reflected in the logged
rows.
...
| Update backend host | Server Config (write) | `9478f8ff-cb2f-4447-8a2f-49791de6bc09` | `{"host":"https://ac.aromen.biz"}` (path stays hardcoded in firmware) |
...
1. **NTP** during any Wi-Fi sync cycle. The corrected time is written
   back to the DS1307 if it drifts more than `RTC_WRITEBACK_DRIFT_SEC`
   (default 2 s) from the chip's reading.
2. **DS1307** at boot. If the RTC chip is present and its oscillator is
   running (clock-halt bit clear), the firmware seeds the wall clock
   immediately so "Today: X kWh" totals track from the first second.
3. **MilesWeb `server_time`** in the ingest response. If both the
   DS1307 and NTP failed, the device still POSTs (with empty
...
4. **BLE Set Wall Time** characteristic
   ...
   DS1307 is absent or reports lost-power, the phone time also seeds
   the RTC.

The Device Info characteristic exposes `rtc_ok` (true if the DS1307 is
present and healthy) and `wall_clock_known` (true after any source has
landed). On a fresh DS1307 (CR2032 battery just installed), the chip's
oscillator stays halted until the first NTP or BLE writeback restarts it
(clearing the clock-halt bit) — after that, power-cycling the ESP32 still
preserves time.
