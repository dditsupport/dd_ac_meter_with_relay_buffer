...
case 'list':
    $rows = $pdo->query(
        'SELECT u.id, u.username, u.email, u.is_admin, u.created_at, u.last_login_at,
                (SELECT COUNT(*) FROM ed_energy_devices d WHERE d.owner_user_id = u.id) AS device_count
           FROM ed_users u ORDER BY u.username'
    )->fetchAll();
...
case 'set_admin':
    ...
    $pdo->prepare('UPDATE ed_users SET is_admin = ? WHERE id = ?')
...
case 'delete':
    ...
    $pdo->prepare('DELETE FROM ed_users WHERE id = ?')->execute([$user_id]);
    json_response(200, ['ok' => true]);
...
function create_user_record(
    ...
    try {
        $pdo->prepare(
            'INSERT INTO ed_users (username, password_hash, email, is_admin) VALUES (?, ?, ?, ?)'
...
function update_user_password(
    ...
    $pdo->prepare('UPDATE ed_users SET password_hash = ? WHERE id = ?')
